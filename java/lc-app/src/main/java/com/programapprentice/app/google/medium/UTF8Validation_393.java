...
public class UTF8Validation_393 {
    ...
    public boolean startsWithOneZero(int num) {
        int tmp = num >> 6;
        return tmp == 2;
    }
    ...
    public boolean statsWithZero(int num) {
        int tmp = num >> 7;
        return tmp == 0;
    }
    ...
    public boolean validUtf8(int[] data) {
        ...
        while (idx < data.length) {
            int cur = data[idx];
            if (numOfCode == 0) {
                if (statsWithZero(cur)) {
                    idx++;
                    continue;
                } else {
                    int numOfOnes = getNumberOfOne(cur);
                    if (numOfOnes > 4 || numOfOnes == 1) {
                        return false;
                    }
                    numOfCode = numOfOnes - 1;
                }
            } else {
                if (startsWithOneZero(cur)) {
                    numOfCode--;
                } else {
                    return false;
                }
