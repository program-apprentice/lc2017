...
public class OutputContestMatches_544 {
    /**
     ...
     more interesting. Now, you're given n teams, you need to output their final contest matches in the form
     of a string.
    ...
    public String findContestMatch(int n) {
        ...
        for(int i = 1; i <= n; i++) {
            strs[i-1] = String.format("%d", i);
        }
        while(n > 1) {
            for(int i = 0; i < n/2; i++) {
                strs[i] = String.format("(%s,%s)", strs[i], strs[n-i-1]);
            }
            n /= 2;
        }
        return strs[0];
