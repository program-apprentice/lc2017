...
public class FreedomTrail_514 {
    /**
     ...
     which also counts as 1 step. After the pressing, you could begin to spell the next character in the key
    ...
    public int findRotateSteps(String ring, String key) {
        ...
        int[][] opts = new int[keyLength+1][ringLength];
            ...
            for(int j = 0; j < ringLength; j++) {
                ...
                for(int k = 0; k < ringLength; k++) {
                    if (ring.charAt(k) == key.charAt(i)) {
                        ...
                        opts[i][j] = Math.min(opts[i][j], step + opts[i+1][k]);
                    }
                }
            }
        }
        return opts[0][0] + keyLength;
    }
}
