...
public class ShoppingOffers_638 {
     ...
     * */
    private boolean isUsable(List<Integer> offer, List<Integer> needs) {
    ...
    private List<Integer> getRemains(List<Integer> offer, List<Integer> needs) {
    ...
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        ...
        int normalSpend = noOffer(price, needs);
        for(List<Integer> offer : special) {
            if(isUsable(offer, needs)) {
                usedOffer = true;
                List<Integer> remains = getRemains(offer, needs);
                spend = Math.min(offer.get(offer.size()-1) + shoppingOffers(price, special, remains), spend);
            }
        }
        if (!usedOffer) {
            spend = 0;
        ...
        return Math.min(spend, normalSpend);
    }
}
